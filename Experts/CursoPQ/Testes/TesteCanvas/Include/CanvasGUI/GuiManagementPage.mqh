...
private:
   ...
   void Focus(const int id)
     {
      m_focus=id;
      for(int i=0;i<2;i++) m_select[i].focused=id==i;
      for(int i=0;i<5;i++) m_text[i].focused=id==i+2;
      m_back.focused=id==7; m_save.focused=id==8; m_dirty=true;
     }
   void Begin(const int id)
     {
      if(id>=2 && id<=6 && !state.Enabled(id)) return;
      Focus(id);
      if(id>=0 && id<2) { m_select[id].Open(m_height); m_open=m_select[id].active ? id : -1; }
      else if(id>=2 && id<7) { m_edit=id-2; m_text[m_edit].Begin(); }
      m_dirty=true;
     }
...
public:
   ...
   void Create()
     {
      ...
      for(int i=0;i<2;i++) { m_select[i].SetOptions("Desativado|Pontos|Porcentagem"); m_select[i].SetSelected(0); }
      UpdateTargets();
   ...
   void ClearHover()
     {
      for(int i=0;i<2;i++) m_select[i].SetHover(false);
   ...
   int Click(const int x,const int y)
     {
      ...
      int hit=-1;
      for(int i=0;i<2;i++) if(m_select[i].ContainsPoint(x,y)) hit=i;
      for(int i=0;i<5;i++) if(m_text[i].ContainsPoint(x,y)) hit=i+2;
      if(m_edit>=0 && hit==m_edit+2) return 0;
      if(!Finish(true)) return 0;
      if(hit>=0) Begin(hit);
      else if(m_back.ContainsPoint(x,y)) { Focus(7); return Ready() ? 2 : 0; }
      else if(m_save.ContainsPoint(x,y)) { Focus(8); return Ready() ? 1 : 0; }
      return 0;
     }
   ...
   int Key(const int key)
     {
      if(key==9)
        {
         ...
         int order[]={0,2,3,1,4,5,6,7,8};
         int position=back ? 9 : -1;
         for(int i=0;i<9;i++) if(order[i]==m_focus) { position=i; break; }
         int next=-1;
         for(position+=back ? -1 : 1;position>=0 && position<9;position+=back ? -1 : 1)
           { int id=order[position]; if(id<2 || id>6 || state.Enabled(id)) { next=id; break; } }
         if(next<0) { Focus(-1); return 3; }
         Focus(next); if(next>=2 && next<7) Begin(next); return 0;
        }
      if(m_open>=0)
        {
      ...
      if(key==13 || key==32)
        {
         if(m_focus==7) return Ready() ? 2 : 0;
         if(m_focus==8) return Ready() ? 1 : 0;
         Begin(m_focus); return 0;
        }
      if(m_focus>=0 && m_focus<2 && (key==38 || key==40)) Begin(m_focus);
      else if(m_focus>=2 && m_focus<7 && ((key>=48 && key<=57) || (key>=96 && key<=105) || key==8 || key==46 || key==189 || key==109 || key==190 || key==188 || key==110))
        { Begin(m_focus); m_text[m_edit].Key(key); m_dirty=true; }
      return 0;
     }
   void EnterFocus(const bool last) { Focus(last ? 8 : 0); }
   void Render(CGuiRenderer &r,const bool full)
     {
      ...
      for(int i=0;i<2;i++)
        {
         ...
         r.Text(c.x+52,c.y+20,i==0 ? "BREAKEVEN" : "TRAILING STOP",GUI_TEXT,14,true,c.w-76);
         r.Text(c.x+24,c.y+c.h-28,"Valores em "+state.Unit(i),GUI_MUTED,12,false,c.w-48);
        }
      for(int id=0;id<7;id++) { m_labels[id].Draw(r); if(id<2) m_select[id].Draw(r); else m_text[id-2].Draw(r); }
      ...
      r.Text(m_summary.x+24,m_summary.y+50,"Breakeven: "+state.Summary(0),GUI_TEXT,12,false,m_summary.w-48);
      r.Text(m_summary.x+24,m_summary.y+76,"Trailing stop: "+state.Summary(1),GUI_MUTED,12,false,m_summary.w-48);
