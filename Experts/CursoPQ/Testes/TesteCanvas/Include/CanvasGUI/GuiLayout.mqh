...
public:
   ...
   void Calculate(const int w,const int h)
     {
      ...
      else { cards[0].Set(left,112,content_width,204); cards[1].Set(left,334,content_width,204); }
   ...
   void IndicatorBounds(const int card,GuiRect &r) { r.Set(cards[card].x+20,cards[card].y+65,cards[card].w-40,40); }
   void ParameterBounds(const int card,const int index,GuiRect &r)
     {
      int w=(cards[card].w-40-(columns-1)*16)/columns;
      r.Set(cards[card].x+20+(index%columns)*(w+16),cards[card].y+143+(index/columns)*72,w,40);
     }
  };
#endif
